...
public class EmployeeControllerTest {
    ...
    public void detailEmployee_expectSuccess() throws Exception {
        Mockito.when(employeeService.getEmployee(any()))
                .thenReturn(ResponseEntity.ok().body(BaseResponse.builder().message(ConstantMess.KEY_SUCCESS).data(getEmployee()).build()));
    ...
    public void findAllEmployee_expectSuccess() throws Exception {
        ...
        Mockito.when(employeeService.listEmployee()).thenReturn(ResponseEntity.ok().body(BaseResponse.builder().message(ConstantMess.KEY_SUCCESS).data(employeeList).build()));
    ...
    public void findAllEmployee_expectError() throws Exception {
        Mockito.when(employeeService.listEmployee()).thenReturn(ResponseEntity.status(HttpStatus.NOT_FOUND).body(BaseResponse.builder().message(ConstantMess.KEY_DATA_NOT_FOUND).data(new ArrayList<>()).build()));
        mockMvc.perform(post("/employee/")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.message", is(ConstantMess.KEY_DATA_NOT_FOUND)));
    ...
    public void updateEmployee_expectSuccess() throws Exception {
        Mockito.when(employeeService.updateEmployee(any()))
                .thenReturn(ResponseEntity.ok().body(BaseResponse.builder().message(ConstantMess.KEY_SUCCESS_UPDATE).data(getEmployee()).build()));
    ...
    public void createEmployee_expectSuccess() throws Exception {
        Mockito.when(employeeService.insertEmployee(any()))
                .thenReturn(ResponseEntity.ok().body(BaseResponse.builder().message(ConstantMess.KEY_SUCCESS_CREATE).data(getEmployee()).build()));
